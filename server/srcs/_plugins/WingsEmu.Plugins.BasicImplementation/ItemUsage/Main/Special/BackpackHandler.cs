...
public class BackpackHandler : IItemHandler
{
    private readonly IGameLanguageService _gameLanguage;

    public BackpackHandler(IGameLanguageService gameLanguage) => _gameLanguage = gameLanguage;

    public ItemType ItemType => ItemType.Special;
    public long[] Effects { get; } = { 1005, 601 };

    public async Task HandleAsync(IClientSession session, InventoryUseItemEvent e)
    {
        if (session.PlayerEntity.HaveStaticBonus(StaticBonusType.Backpack))
        {
            return;
        }
