...
";
        ...
        public static async Task Main(string[] args)
        {
            ...
            using IHost host = CreateHostBuilder(args).Build();
            {
                ...
                // resolve scoped wrapper so underlying handler with scoped deps is created per execution
                ScopedPacketHandler<TypedCredentialsLoginPacketHandler> typedCredentialsHandler = services.GetRequiredService<ScopedPacketHandler<TypedCredentialsLoginPacketHandler>>();
                processor.RegisterHandler(typeof(Nos0575Packet), typedCredentialsHandler);

                // use a scope to resolve scoped services safely
                using IServiceScope scope = services.CreateScope();
                IServerApiService master = scope.ServiceProvider.GetRequiredService<IServerApiService>();
                ...
                try
                {
                    server = new Network.LoginServer(IPAddress.Loopback, port, new SmartSpamProtector(), processor, packetDeserializer);
                    server.Start();
                }
                catch (Exception ex)
                {
